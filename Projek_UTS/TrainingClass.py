...
class TrainingClass:
    ...
    def __init__(self, name, model_path, data_path, save_folder, no_epochs, kernel_size, batch_size, filters, lrate = 1e-4, reg = 0.0001,  loss = 'categorical_crossentropy', duplicate = True ):
        ...
        #if(duplicate == True):
            #self.train, self.train_label = self.duplicate()
    ...
    '''    
    def load_data(self):
        image_list = sorted(os.listdir(self.data_path+'/image'))[:25]
        train_image = []
        for img_name in image_list:
          img = cv2.imread(self.data_path+'/image' + '/' + img_name, 0)
          img = cv2.resize(img, (200,200))
          #img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
          train_image.append(img)
        train_image = np.asarray(train_image)
        n, h, w = train_image.shape
        self.train = train_image.reshape(n, h, w, 1)
        self.no_images, self.height, self.width, self.channels= self.train.shape

        label_list = sorted(os.listdir(self.data_path+'/label'))[:25]
        train_label = []
        for label_name in label_list:
          ...
          cropped_label = label[math.ceil(((1-frac)/2)*y):starty, math.ceil(((1-frac)/2)*x):startx]
          label = cv2.resize(cropped_label,(200, 200))
          train_label.append(label)
        self.train_label = np.asarray(train_label)
        self.no_images, _, _, self.no_classes = self.train_label.shape

        val_image_list = sorted(os.listdir(self.data_path+'/image'))[25:30]
        val_image = []
        for img_name in val_image_list:
          img = cv2.imread(self.data_path+'/image'+ '/' + img_name, 0)
          img = cv2.resize(img, (200,200))
          #img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
          val_image.append(img)
        val_image = np.asarray(val_image)
        n, h, w = val_image.shape
        self.val = val_image.reshape(n, h, w, 1)

        val_label_list = sorted(os.listdir(self.data_path+'/label'))[25:30]
        val_label = []
        for label_name in val_label_list:
          labval =cv2.imread(self.data_path+'/label' + '/' + label_name)
          frac = 0.90
          y,x,c = labval.shape
          startx = math.ceil(x-(((1-frac)/2)*x))
          starty = math.ceil(y-(((1-frac)/2)*y))
          cropped_labval = labval[math.ceil(((1-frac)/2)*y):starty, math.ceil(((1-frac)/2)*x):startx]
          labval = cv2.resize(cropped_label,(200, 200))
          val_label.append(labval)
        self.val_label = np.asarray(val_label)
    ...
    def load_data(self):
      ...
      label_training =[]
      image_training=[]
      image_list = sorted(os.listdir(self.data_path+'/images'))[:140]
      label_list = sorted(os.listdir(self.data_path+'/labels'))[:140]
      #data_list = os.listdir(data_dir)[:15]
      #label_list = os.listdir(label_dir)[:15]

      for image_name in image_list:
        image = cv2.imread(self.data_path+'/images' + '/' + image_name)
      ...
      for label_name in label_list:
        label = cv2.imread(self.data_path+'/labels' + '/' + label_name)
      ...
      self.train = image_training[:120]
      self.no_images, self.height, self.width, self.channels= self.train.shape
      self.train_label = label_training[:120]
      self.no_images, _, _, self.no_classes = self.train_label.shape
      self.val = image_training[120:140]
      self.val_label = label_training[120:140]
      self.val_label = self.val_label.reshape((-1,self.height*self.width,self.no_classes))
      print("Data loaded succesfully.")
